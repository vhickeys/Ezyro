...
class Portfolio
{
    ...
    public function getRelatedPortfolio($id, $portfolio_id)
    {
        $status = 0;
        $sql = "SELECT * FROM portfolios WHERE category_id=? AND id!=? AND status=? ORDER BY date DESC";
        $statement = $this->db->prepare($sql);
        $statement->bindParam(1, $id, PDO::PARAM_INT);
        $statement->bindParam(2, $portfolio_id, PDO::PARAM_INT);
